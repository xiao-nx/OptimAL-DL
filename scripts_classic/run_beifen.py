...
def run():
    ...
    # selecte network  
    # Maxpool
    if config.NETWORK == 'textCNN':
        model = textcnn.textCNN(vocab_size, vec=pretrained_vec)
    elif config.NETWORK == 'textRNN':
        model = textRNN.textRNN(vocab_size, vec=pretrained_vec)
    elif config.NETWORK == 'textRNN_Att':
        print('textRNN_Att training start...')
        model = textRNN_Att.textRNN_Att(vocab_size, vec=pretrained_vec)
    elif config.NETWORK == 'DPCNN':
        print('DPCNN training start...')
        model = DPCNN.DPCNN(vocab_size,embedding_pretrained=embedding_pretrained_matrix)
    else:
